...
LKEY=/home/greg/.ssh/Third_AWS.pem
...
up:
	aws --region $(REGION) --output json ec2 run-instances \
		...
		--key-name $(KEY) \
                --tag-specifications 'ResourceType=instance,Tags=[{Key=mnemonic-name,Value='`curl --silent $(NAMING_SVC)`'}, {Key=course,Value='c756'}, {Key=ssh-user,Value='$(SSH_USER)'}]' \
